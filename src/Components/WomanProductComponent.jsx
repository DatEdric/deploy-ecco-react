...
export default function WomanProductComponent() {
    const { data } = useContext(Context);
    const [shoes, setShoe] = useState([]);
    const womanProducts = data.filter((a) => a.id === 1);
    ...
    const handleBestsellerFilter = () => {
        const shoe = womanProducts.map((i) => i.products.filter((value) => value.bestseller === true));
        setShoe(shoe);
    };
    // lọc sản phẩm là phụ kiện
    const handleAccessoryFilter = () => {
        const shoe = womanProducts.map((i) => i.products.filter((value) => value.type === "accessory"));
        setShoe(shoe);
    };
    // lọc các sản phẩm là giày
    const handleShoeFilter = () => {
        const shoe = womanProducts.map((i) => i.products.filter((value) => value.type === "shoes"));
        setShoe(shoe);
    };
    return (
        <div className="">
            <div className="banner">
                <img src="public/images/woman_banner.webp" alt="" />
            </div>

            <h2 className="title_page text-center mt-4 ">Nữ</h2>
                    ...
                    <div className="display_product d-inline-flex w-50">
                        ...
                        <span className="total-product">
                            ...
                            {filteredProducts.map((i) => i.length) == 0
                                ? womanProducts.map((i) => i.products.length)
