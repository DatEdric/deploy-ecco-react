...
export default function GolfComponent() {
    const { data } = useContext(Context);
    const [golf, setGolf] = useState([]);
    const golfProducts = data.filter((i) => i.id === 3);
    ...
    const handleBestsellerFilter = () => {
        const shoe = golfProducts.map((i) => i.products.filter((value) => value.bestseller === true));
        setGolf(shoe);
    };
    // lọc sản phẩm là phụ kiện
    const handleAccessoryFilter = () => {
        const shoe = golfProducts.map((i) => i.products.filter((value) => value.type === "accessory"));
        setGolf(shoe);
    };
    // lọc các sản phẩm là giày
    const handleShoeFilter = () => {
        const shoe = golfProducts.map((i) => i.products.filter((value) => value.type === "shoes"));
        setGolf(shoe);
    };
    ...
    const filteredProducts = golf.map((value) => {
    ...
    return (
        <div className="">
            <div className="banner">
                <img src="public/images/golf-slider.webp" alt="" />
            </div>

            <h2 className="title_page text-center mt-4 ">GOLF</h2>
                ...
                <div className="display_product d-inline-flex w-50">
                    ...
                    <span className="total-product">
                        ...
                        {filteredProducts.map((i) => i.length) == 0
                            ? golfProducts.map((i) => i.products.length)
